...
export const EntityList: React.FC<Props> = ({}) => {
  ...
  const getNameCell = (row: EntitySummary) => <Link>{row.displayName}</Link>;
  const getStatusCell = (row: EntitySummary) => (
    <StatusLabel statusType={StatusTypeForLifecycleState[row.lifecycleState]}>
      {ListPrettyLifecycleState(row.lifecycleState as any)}
    </StatusLabel>
  );

  const getTimeCreatedCell = (row: EntitySummary) => getDateTimeFormat(row.timeCreated);
  const getRowId = (row: EntitySummary) => row.id;
  const getDescriptionCell = (row: EntitySummary) =>
